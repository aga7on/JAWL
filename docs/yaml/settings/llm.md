...
* **`thinking_policy`**: Optional control for providers that expose an `enable_thinking` request extension. `provider_default` sends no extension, `always` and `never` force it, and `first_step` enables deep Thinking only for the first decision in each ReAct cycle. The latter is useful with QWB: planning keeps Qwen Thinking, while routine read/edit/verify follow-ups avoid paying the full Thinking latency again. Providers that reject unknown extensions should use `provider_default`.
