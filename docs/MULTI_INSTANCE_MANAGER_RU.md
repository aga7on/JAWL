...
Откройте `python jawl.py` и выберите `Agents` на главном dashboard. Реальные
горячие клавиши отображаются рядом с пунктами и работают вместе со стрелками.
Основной `default`-агент больше не показан как отдельный `legacy`-механизм: в
интерфейсе это обычный агент с ролью `Primary`.
