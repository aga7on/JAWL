...
1. Introduce cycle/task trace IDs that link LLM calls, actions, ticks,
   verification runs, and commits without replacing JAWL's event model.
2. Make Git diff collection byte-bounded while streaming and add a repository
   verification policy file with an explicit command allowlist.
3. Add an adapter that can expose selected registered skills as native tool
   schemas while retaining `execute_skill` for Qwen and legacy providers.
4. Upgrade code navigation with optional tree-sitter/LSP-backed definitions,
   references, and dependency slices; retain the current zero-index fallback.
5. Add fixed-task end-to-end evaluation repositories and grade patch correctness,
