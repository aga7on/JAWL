...
| Verification | Strong | Detected allowlisted profiles, hashed repository policy, process-tree timeout, exact-state fingerprint commit gate, opt-in bounded stability runs with fail-closed flaky classification, and opt-in transitive Python affected-test selection with durable decision evidence and full-suite fallback on uncertainty | No historical duration-aware sharding or learned selection beyond the deterministic import graph |
