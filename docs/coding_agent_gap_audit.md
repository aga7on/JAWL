...
| Delegated work | Good | Bounded durable status registry, redacted task summaries, restart interruption classification, exact-handle cancel, shutdown draining, identity-bound reports, and success/error/cancel lifecycle evidence | Completed reports are notified but not yet automatically reconciled into a parent coding plan |
...
| --- | --- | --- |
| Parallel delegated work | [Codex subagents](https://developers.openai.com/codex/subagents/), [Claude Code subagents](https://code.claude.com/docs/en/sub-agents) | Swarm roles now have durable inspect/cancel/restart state, identity-bound reports, lifecycle evidence, and shutdown draining; parent-plan result reconciliation still needs benchmark coverage |
