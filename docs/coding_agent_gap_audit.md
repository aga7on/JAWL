...
| Verification | Strong | Detected allowlisted profiles, hashed repository policy, process-tree timeout, exact-state commit gate, bounded stability runs, fail-closed flaky classification, transitive Python affected-test selection, and repository-scoped duration-aware pytest worker lanes with durable partial evidence | Selection is deterministic rather than learned beyond imports; no distributed cross-host sharding or pytest node-level timing |
...
| Desktop application control | Strong on Windows | Bounded UIA window/control observation, accessibility properties, ephemeral exact-state element references, stale-target refusal, semantic invoke/click/focus/value/toggle/select/expand actions, explicit dispatch-vs-verification, postcondition waits, plus screenshot/coordinate/keyboard fallbacks | Secure desktop/elevated apps remain OS-protected; custom canvases require vision/coordinates; macOS/Linux do not yet have equivalent semantic accessibility adapters |
