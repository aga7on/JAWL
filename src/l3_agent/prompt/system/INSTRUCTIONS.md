...
- Build a compact repository map, use `locate_code_symbol` for bounded definition-first occurrences, then read only the required numbered line ranges. Treat `lexical_fallback` results as candidates that require inspection; do not inject whole large files when precise context is available.
